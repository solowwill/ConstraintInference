...
class cmdp:
    ...
    @staticmethod
    def gen_baseline(cmdp, targ_performance, dec=.02):
        ...
        while pib_perf > targ_performance:
            ...
            while flag:
                ...
                if np.sum(mdp.P[s,a]) == 0:
                    print('chose bad action')
                    continue
                pi_b[s,best_a] -= dec
                pi_b[s,a] += dec
...
def main():
    ...
    constraints = np.array([np.array([0,1]), np.zeros(loaded_mdp.S), np.zeros(loaded_mdp.A) ],dtype=object)
    np.save(f'{path}configs/{env_type}/{env}_constraints.npy', constraints)

    constrained_mdp = cmdp(loaded_mdp, constraints)

    b = cmdp.gen_baseline(constrained_mdp, baseline)
    ...
    for i in range(num_d):
        ...
        D = cmdp.build_D(constrained_mdp, b, d_size)

        np.save(f'{path}config_data/{env_type}/{env}/b{baseline}/ds_{i}.npy', np.array(D,dtype=object))
