...
class utils:
    ...
    def parse_args():
        ...
        # ICRL params
        parser.add_argument("--icrl_type", type=str, default="bicrl")
        parser.add_argument("--dkl", type=float, default=1.0)
...
def main():
    ...
    constraints = np.load(f'{path}configs/{env_type}/{env}_constraints.npy', allow_pickle=True)

    # Build the constraints
    constrained_mdp = cmdp(loaded_mdp, constraints)
    num_feats = len(constraints[0])
    ...
    dsizes = [10, 20, 50, 100, 200, 500, 1000]
    #dsizes = [10, 20, 50, 100, 200]
    baselines = [.6, .8, 1]


    data = np.empty(shape=(len(dsizes), len(baselines), num_d, 5))

   
    for j in range(len(baselines)):
        for i in range(num_d):
            D = np.load(f'{path}config_data/{env_type}/{env}/b{baselines[j]}/ds_{i}.npy', allow_pickle=True)
            ...
            for k in range(len(dsizes)):
                ...
                if args.icrl_type == 'bicrl':
                    agent = agents.BICRL_Agent(exp_mdp, D[:dsizes[k]], num_samples=args.iterations)
                else:
                    agents.MAXEntICRL_Agent(exp_mdp, D[:dsizes[k]], dkl=args.dkl)
                learned_constraints = agent.icrl()
    ...
    np.save(f'data/{env_type}/{env}/bayesian_data_{num_d}.npy', data)
    ...
    for i in range(len(baselines)):
        ...
        ax[0,0].plot(dsizes, data_mean[:,i,0], label=f'Baseline={baselines[i]}')
        ax[0,0].fill_between(dsizes, data_std[:,i,0], alpha=.5)
        ax[0,0].set_ylabel('Precision')

        ax[0,1].plot(dsizes, data_mean[:,i,2], label=f'Baseline={baselines[i]}')
        ax[0,1].fill_between(dsizes, data_std[:,i,2], alpha=.5)
        ax[0,0].set_ylabel('Accuracy')

        ax[1,0].plot(dsizes, data_mean[:,i,3], label=f'Baseline={baselines[i]}')
        ax[1,0].fill_between(dsizes, data_std[:,i,3], alpha=.5)
        ax[0,0].set_ylabel('False Positive Rate')

        ax[1,1].plot(dsizes, data_mean[:,i,4], label=f'Baseline={baselines[i]}')
    ...
    plt.savefig(f'data/{env_type}/{env}/bayesian_data_{num_d}_fig.png')
    plt.show()
