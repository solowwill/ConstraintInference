...
class cmdp:
    ...
    def build_constrained_reward(self):
        ...
        if len(self.mdp.R.shape) == 2:
            self.C = np.zeros((self.mdp.S, self.mdp.A))
        ...
        if len(self.mdp.R.shape) == 3:
            self.C = np.zeros((self.mdp.S, self.mdp.A, self.mdp.S))
...
def main():
    ...
    data = np.empty(shape=(len(dsizes), len(dkls), num_d, 5))

    for k in range(len(dsizes)):
       
        for j in range(len(dkls)):
            print(f'\n\n{dsizes[k]}, {dkls[j]}')
            for i in range(num_d):
                ...
                max_ent_agent = agents.MAXEntICRL_Agent(exp_mdp, D, dkl=dkls[j])

                c_hat = max_ent_agent.icrl()
                print(c_hat)
                learned_constraints = np.zeros(num_feats+loaded_mdp.S+loaded_mdp.A)
                if len(c_hat) != 0:
                    learned_constraints[c_hat] = True
    ...
    np.save(f'data/{env_type}/{env}/maxent_b{baseline}_data.npy', data)
    ...
    plt.title('MaxEnt ICRL')

    plt.savefig(f'data/{env_type}/{env}/maxent_fig.png')
    plt.show()
